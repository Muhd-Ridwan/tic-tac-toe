...
export default function Player({ name, symbol, isActive, onChangeName }) {
  ...
  function handleClick() {
    ...
    setEditing((wasEditing) => {
      if (wasEditing) {
        onChangeName(symbol, draftName);
      } else {
        setDraftName(name);
      }
      return !wasEditing;
    }); // This one is the correct way to update state based on old state
  }
  ...
  let editPlayerName = <span className="player-name">{name}</span>;
  //let buttonText = "Edit";

  if (editing) {
    editPlayerName = (
      <input type="text" required value={draftName} onChange={handleChange} />
    );
    //buttonText = "Save";
  }
